rule capability_process_spawn
{
    meta:
        ...
        max_hits = 1
        path_include = "*.py,*.pyx,*.pyi,*.pth,*.js,*.ts,*.jsx,*.tsx,*.mjs,*.cjs,*.go,*.rb,*.gemspec,extconf.rb,*/extconf.rb,Rakefile,*/Rakefile,*.cpp,*.cc,*.cxx,*.h,*.hpp,*.cs,*.rs"
    strings:
        // Python - subprocess
        ...
        // C++ - POSIX/libc process spawning
        $cpp_system = /[^.\w]system\s*\(/ nocase
        $cpp_popen = /[^.\w]popen\s*\(/ nocase
        $cpp_exec_family = /\bexecl?p?e?v?\s*\(/ nocase
        $cpp_fork = /[^.\w]fork\s*\(\s*\)/ nocase
        $cpp_createprocess = /\bCreateProcess[AW]?\s*\(/ nocase
        $cpp_shellexecute = /\bShellExecute[AW]?\s*\(/ nocase

        // C# / .NET - Process API
        $cs_process_start = /Process\s*\.\s*Start\s*\(/ nocase
        $cs_processstartinfo = /\bProcessStartInfo\s*\(/ nocase
        $cs_powershell_create = /PowerShell\s*\.\s*Create\s*\(/ nocase

        // Rust - std::process::Command
        $rs_command_new = /Command\s*::\s*new\s*\(/ nocase
        $rs_command_output = /\.\s*(spawn|output|status)\s*\(\s*\)/ nocase
